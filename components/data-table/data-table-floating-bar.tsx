...
import { ArrowUpIcon, CheckCircledIcon, Cross2Icon, TrashIcon } from '@radix-ui/react-icons'
...
import { TaskPriority, TaskStatus, priorityEnum, statusEnum } from '@/app/(app.domain.com)/dashboard/membership/_components/tasks'
import { updateTaskPriority, updateTaskStatus } from '@/actions/tasks'
...
export function DataTableFloatingBar<TData>({ table, deleteRowsAction, className, ...props }: DataTableFloatingBarProps<TData>) {
  if (table.getFilteredSelectedRowModel().rows.length <= 0) return null

  function updateTasksStatus(table: Table<TData>, status: string) {
    const selectedRows = table.getFilteredSelectedRowModel().rows as unknown as { original: Tasks }[]
  ...
  return (
    <div className={cn('mx-auto flex w-fit items-center gap-2 rounded-md bg-zinc-900 px-4 py-2 text-white', className)} {...props}>
      <Button
      ...
      <Select onValueChange={(value) => updateTasksPriority(table, value)}>
        <SelectTrigger asChild>
          <Button aria-label='Delete selected rows' title='Priority' className='h-auto bg-transparent p-1 text-white hover:bg-zinc-700'>
