...
export function DataTableMultiFilter<TData>({ table, allOptions, options, setSelectedOptions }: DataTableMultiFilterProps<TData>) {
  const [open, setOpen] = React.useState(true)
  const [operator, setOperator] = React.useState<(typeof operators)[number] | undefined>(operators[0])
...
export function MultiFilterRow<TData>({
  ...
  const filterVarieties = selectedOption?.items.length ? ['is', 'is not'] : ['contains', 'does not contain', 'is', 'is not']
  ...
  return (
    <div className='flex items-center space-x-2'>
      {i === 0 ? (
        ...
        <Select value={operator?.value} onValueChange={(value) => setOperator(operators.find((o) => o.value === value))}>
