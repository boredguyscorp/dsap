...
export default function MDX({ source }: { source: MDXRemoteProps }) {
  ...
  return (
    <article className={`prose-md prose w-11/12 dark:prose-invert sm:w-full ${styles.root}`} suppressHydrationWarning={true}>
      {/* @ts-ignore */}
...
interface ExampleCardProps extends Pick<Post, 'description' | 'image' | 'imageBlurhash'> {
  name: string | null
  url: string | null
}
    ...
    <a href={`https://${data.url}`} target='_blank' rel='noreferrer'>
      ...
      <div className='ease flex h-36 items-center overflow-hidden rounded-xl border-2 border-gray-100 bg-white transition-all duration-200 focus:border-black active:border-black md:h-48 lg:hidden'>
        ...
        <div className='w-3/5 px-5 py-6'>
          <h3 className='font-cal my-0 truncate text-xl font-bold tracking-wide dark:text-white'>{data.name}</h3>
          <p className='mt-3 text-sm font-normal italic leading-snug text-gray-800'>{data.description}</p>
        </div>
      </div>
    </a>
  )
